...
// Do: validate once so a negative range can't be written; derive end when needed
type NonNegativeDurationMs = number & { readonly __brand: "NonNegativeDurationMs" };

function parseNonNegativeDurationMs(input: number): NonNegativeDurationMs {
