...
- Issue Polling executes one Refresh Job at a time; lifecycle jobs may run concurrently up to a fiber budget derived from max concurrent OpenCode sessions. OpenCode `start`/`continue` for lifecycle work is capped by harness Config `maxConcurrentOpencodeSessions` (default 2). Claims use five-minute visibility; Refresh Job reconciliation failures are terminal, while an abandoned claim may be redelivered once after interruption
