...
/**
 * Fail-closed Thinking Level check for a later Agent Turn that uses an
 * explicit profile. Empty catalogs carry no membership information.
 */
export const explicitProfileThinkingLevelViolation = (input: {
  readonly backendLabel: string
  readonly catalog: ReadonlyArray<ExecutionProfileCatalogModel>
  ...
  const byId = new Map(input.catalog.map((model) => [model.id, model] as const))
  const checked: ReadonlyArray<readonly [string, string, string | null]> = [
  ...
  for (const [role, model, thinkingLevel] of checked) {
    if (thinkingLevel === null) continue
    const catalogModel = byId.get(model)
    if (catalogModel === undefined) continue
    if (!catalogModel.thinkingLevels.includes(thinkingLevel)) {
      return thinkingLevelViolationMessage({
        role,
        model,
        thinkingLevel,
        backendLabel: input.backendLabel,
