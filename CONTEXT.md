...
**Refresh Job**:
A durable request for the Issue Reconciler to reconcile one Repository. Acceptance of a Refresh Job does not mean reconciliation has completed. After reconciliation succeeds, unfinished Work Items that own a Work Item PR are inspected for merge outcomes: a merged PR advances local cleanup toward Complete (including Work Items paused because the Issue closed while the PR was open); closed-unmerged Abandon remains limited to merge-related Needs Human. Refresh does not auto-Start a Work Item paused for a closed Issue with an open PR.
