...
**Lifecycle Step**:
The next action required for a Work Item: Create Worktree, Install Dependencies, Implement, Pre-Commit, Review, or a terminal Complete, Failed, or Abandoned state. A successful step advances the Work Item; a failed step leaves the same action pending.
...
**Complete Work Item**:
A terminal Work Item for which Create Worktree, Install Dependencies, Implement, Pre-Commit, and Review all executed successfully. Complete does not mean review approved the changes, a pull request exists, or GitHub closed the Issue.
