...
**Auto-merge**:
A Repository setting that, when enabled, lets Decide PR Merge ask whether a clanker may merge a low-risk PR; when disabled, Decide PR Merge always requires a human unless a true Work Item Auto-merge override is present. Enabling Auto-merge does not itself merge pull requests; only a subsequent Merge PR step merges when Decide PR Merge chooses clanker merge. A successful, non-empty Forge status-check aggregate is required before any harness-initiated merge; the absence of checks or a remaining Expected PR Status Check is not successful verification. It applies only when a Work Item has a pull request and does not gate Close Issue for a No-Change Outcome. Distinct from Merge Mode Always and from a Work Item Auto-merge override.
_Avoid_: Automerge (GitHub product), auto-approve

**Work Item Auto-merge override**:
A nullable durable Work Item policy for Decide PR Merge. `null` follows the live Repository Auto-merge setting; `true` permits the existing risk-assessed Auto-merge flow for this Work Item even if Repository Auto-merge is later disabled; `false` requires human merge even if Repository Auto-merge is later enabled. A true override changes merge ownership but does not relax the requirement for a successful, non-empty Forge status-check aggregate before harness-initiated merge. Implement With persists a concrete true or false override; other creation paths leave it null. Distinct from Merge Mode Always, which skips Decide PR Merge.
_Avoid_: Merge Mode Always, per-Issue Auto-merge, publication policy (as an execution-profile field)

**Merge Mode**:
Durable Work Item policy for post-check merge routing. `ordinary` follows Repository Auto-merge and Decide PR Merge. `always` skips Decide PR Merge (no agent risk decision) and advances to Merge PR after the normal pre-merge lifecycle settles, without bypassing status checks, automated-review handling, conflict resolution, merge revalidation, Forge requirements, or technical Needs Human outcomes. An absent, Expected, pending, or failed status-check aggregate does not satisfy that gate and cannot advance an `always` Work Item to Merge PR. No-Change Outcomes still close the Issue without merge-related steps. Stored on the Work Item and survives restarts. A merge-related Needs Human handoff reached before the mode became `always` is not revoked. Distinct from a Work Item Auto-merge override, which still runs Decide PR Merge.
...
**Implement With**:
An explicit operator request for an Actionable Issue that chooses an Explicit Work Item Execution Profile plus a concrete Work Item Auto-merge override and an optional Implement Locally inspection pause, then creates the Work Item through the same Worker Slot admission path as Implement Now. Its ephemeral `Implement issue #N with...` dialog is pre-filled from the Repository's effective backend, currently resolved model selections, and live Auto-merge setting; model drafts stay backend-scoped while the two checkboxes stay dialog-wide; Implement Locally starts unchecked. A missing default build model leaves a required blank choice, and a failed backend selection does not disable choosing another backend. It requires a non-empty model catalog; submission atomically revalidates Issue eligibility, activates and inspects the chosen backend when necessary, validates catalog membership and the complete profile, persists the profile, override, and pause target together, and creates nothing on failure. Callers that omit the options keep repository-inherited Auto-merge and the remote path. The dialog has Implement and Cancel actions and ordinary loading and failure feedback, but no recovery, recheck, fallback, or retry workflow. It is offered only as `Implement with...` directly below Implement now in the Issue actions menu and does not change Harness Config or Repository settings.
...
The Lifecycle Step that observes the Work Item PR for merge conflicts, Status Check Handoffs, and settled completion conditions. After the Check-Start Deadline, a ready PR on an autonomous merge path that still reports no checks or only Expected PR Status Checks enters Needs Human rather than Decide PR Merge or Merge PR.
...
The instant 90 seconds after the latest Check-Start Anchor, before which neither an absence of checks nor an all-terminal observed set proves startup is complete. At or after the deadline, the harness assumes every check has started, while checks already pending are still watched until they finish. For an autonomous merge path, `no_checks` and Expected PR Status Checks at or after the deadline enter Needs Human rather than advancing to Decide PR Merge or Merge PR.
_Avoid_: No-check grace, Watch residence time, check completion timeout

**Ready-Phase Status Check Round**:
The fresh PR Status Check observation period after a pull request known to the harness as draft becomes ready for review. It protects repositories whose ready-for-review transition can start additional checks. A Repository may omit this round when its settled, non-failing draft-phase checks are sufficient evidence, allowing Mark PR Ready for Review to advance directly to Decide PR Merge when those checks are successful, or to Needs Human when an autonomous merge path still has no successful checks. This omission does not shorten startup deadlines after PR creation, a head push, a check restart, or an automated-review rerun, and it never ignores an observed pending check or bypasses a known aggregate failure.
