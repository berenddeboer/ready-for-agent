...
**Implement With**:
An explicit operator request for an Actionable Issue that chooses an Explicit Work Item Execution Profile and then creates the Work Item through the same remote implementation and Worker Slot admission path as Implement Now. Its ephemeral `Implement issue #N with...` dialog is pre-filled from the Repository's effective backend and currently resolved model selections, keeps backend-scoped drafts while open, and permits any shipped Agent Backend. A missing default build model leaves a required blank choice, and a failed backend selection does not disable choosing another backend. It requires a non-empty model catalog; submission atomically revalidates Issue eligibility, activates and inspects the chosen backend when necessary, validates catalog membership and the complete profile, and creates nothing on failure. The dialog has Implement and Cancel actions and ordinary loading and failure feedback, but no recovery, recheck, fallback, or retry workflow. It is offered only as `Implement with...` directly below Implement now in the Issue actions menu and does not change Harness Config or Repository settings.
