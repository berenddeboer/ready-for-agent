...
A nullable durable Work Item policy for Decide PR Merge. `null` follows the live Repository Auto-merge setting; `true` permits the existing risk-assessed Auto-merge flow for this Work Item even if Repository Auto-merge is later disabled; `false` requires human merge even if Repository Auto-merge is later enabled. Implement With persists a concrete true or false override; other creation paths leave it null. Distinct from Merge Mode Always, which skips Decide PR Merge.
