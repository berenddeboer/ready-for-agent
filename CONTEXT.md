...
**Invocation Boundary**:
The invocation-scoped execution ownership the Harness establishes before an Agent Turn or native repository command can spawn children. On Linux it is a dedicated cgroup v2 so descendants remain owned after setsid, double-fork, or reparenting. Timeout and Interrupt terminate every process in that boundary before a replacement attempt runs against the same Work Item. A new Agent Turn on the same Session gets a new boundary. Independent Work Items keep independent boundaries.
_Avoid_: process group (as the ownership guarantee), tree kill, global execution lock
