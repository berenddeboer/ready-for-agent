...
**Pause Work Item**:
An explicit operator request that marks an unfinished Work Item paused so it will not start further Lifecycle Steps until Start. A running Step Run is not interrupted; after it finishes, the next step is neither enqueued nor started while paused. Step Runs still queued (not running) are cancelled. If the Work Item was Admitted and no Step Run is running, Pause releases its Worker Slot immediately; if a Step Run is still running, the Worker Slot is held until that Step Run finishes, then released. Pause is idempotent when already paused and is rejected for missing, terminal, or Waiting for blockers Work Items (the hold already means do not run; cancel with Reset). A paused Work Item remains unfinished and still blocks Implement Now and Queue for that Issue. The harness may also apply Pause Work Item when Issue revalidation finds the Issue closed or missing while the Work Item still owns a Work Item PR that is open, closed unmerged, or of indeterminate lifecycle status, recording an operator-visible reason for that branch; a confirmed merged PR advances to local cleanup instead. Start after reopening the Issue resumes the current Lifecycle Step. Closed-unmerged Abandon remains limited to merge-related Needs Human (Decide PR Merge / Merge PR). Distinct from Repository Paused.
...
**Complete Work Item**:
A terminal Work Item whose remote outcome is finished and whose local cleanup has finished. For changed work, the Work Item PR is merged, either by the harness after a clanker Decide PR Merge decision or by a human (or other external merge) before a Refresh Job resumes cleanup—including when that merge supersedes an unfinished operational Lifecycle Step such as Watch or Investigate PR Status Checks. Closing the linked Issue as part of the merge does not turn the Work Item into a Failed Work Item. For a No-Change Outcome, the GitHub Issue is closed without a pull request.
