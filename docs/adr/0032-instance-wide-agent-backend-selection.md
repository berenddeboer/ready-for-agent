...
Every Work Item captures the Active Agent Backend as provenance alongside its backend-local build/review Agent Models and optional Thinking Levels. Existing records are migrated as OpenCode, backend-specific `variant` and concurrency names are replaced end-to-end, and model selections are atomic: omitting a model inherits the whole parent selection, while choosing a model without a Thinking Level uses that model's backend default.

## Consequences

Backend selection is a product setting but activation is a restart boundary because the application runtime is layered around one adapter. Captured provenance is visible on Work Item detail and completed-history/session views, but does not imply per-Work-Item routing.
