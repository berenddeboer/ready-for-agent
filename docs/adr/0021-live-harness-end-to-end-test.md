...
| `harness:e2e-ui-history` | `@ui-history` only | `KEYMAXXER_ENABLED=false` (no fixture vault or clone) | Settings history, Repository settings history, Session Telemetry history, and Kanban that seeds a Paused Repository. May still use ambient OpenCode for Settings catalog coverage. |
