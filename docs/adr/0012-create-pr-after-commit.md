...
After Commit succeeds and before Watch PR Status Checks, the Create PR Lifecycle Step ensures an open draft pull request exists for the Work Item branch. The harness attempts the operation natively first: it reuses an existing open PR for the exact Work Item branch when found (and, while that PR is still a draft, reconciles its title and body to the Work Item's canonical publication copy — the post-rewrite body Commit persisted); otherwise it pushes the branch, creates a draft PR through the harness-owned GitHub service with the same canonical title and body as the commit (including the normalized closing reference and any rewritten user-attachment image URLs), using the harness credential path without exposing raw tokens to an Agent Turn. Create PR does not upload attachments or invent a second description. Success requires resolving the open PR identity and persisting its number on the Work Item.
...
- Commit subject/body and PR title/body are identical canonical publication copy (the post-rewrite body persisted by Commit).
