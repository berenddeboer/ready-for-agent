...
Waiting for GitHub also releases its Worker Slot. The release immediately runs the ordinary FIFO waiter admission pass before the delayed GitHub wake is due; the later wake re-enters the same admission path and therefore never bypasses existing Worker Slot waiters.
